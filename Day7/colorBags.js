let rules = `light green bags contain 2 pale cyan bags.
...
// bags which contain shiny gold bag
for (let rule of rules) {
    let bag = rule.split(" bags contain ");
    if (bag[1].includes("shiny gold")) {
        if (!colorBags.includes(bag[0])) {
            colorBags.push(bag[0]);
        }
    }
}

// bags which contain bags which can contain atleast one shiny gold bag
for (let rule of rules) {
    let bag = rule.split(" bags contain ");
    // console.log(bag[1]);
    for (let color of colorBags) {
        if (bag[1].includes(color)) {
            console.log("Gotchaaaa >>>>" ,bag[1]);
            if(!indirect.includes(bag[0])) {
                indirect.push(bag[0]);
            }
        }
    }
...
console.log(colorBags,indirect);