const passwords = `15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
...
function isValid() {
    ...
    let valid = 0;
    for (let pass of passwordsList) {
        const firstPos = pass.split(" ")[0].split("-")[0];
        const secondPos = pass.split(" ")[0].split("-")[1];
        const char = pass.split(" ")[1][0];
        const password = pass.split(" ")[2];
        let numOcc = 0;
        if ((password[firstPos - 1] === char && password[secondPos - 1] !== char) || (password[firstPos - 1] !== char && password[secondPos - 1] === char))
            ++valid;
    }
    return valid;
}
