const passports = `eyr:2029 iyr:2013
...
// Get each passport details
for (let pprt of passportArr) {
    pprt = pprt.replace(/\n|\s/g, ",").split(",");
    console.log(pprt);
    let criteriaMeet = 0;
    let passObj = {};
    for (let elem of pprt) {
        console.log(elem);
        let key = elem.split(":")[0];
        let val = elem.split(":")[1];
        passObj[key] = val;
        // Check if valid vals for keys
        if(key === "cid") {
            continue;
        }
        else if (key === "byr") {
            ...
            // console.log(val);
            if (val.includes("in")) {
                if (Number(val.split("in")[0]) >= 59 && Number(val.split("in")[0]) <= 76)
                    criteriaMeet++;
            }
            else if (val.includes("cm")) {
                if (Number(val.split("cm")[0]) >= 150 && Number(val.split("cm")[0]) <= 193)
                    criteriaMeet++;
            }
        }
        else if (key === "hcl") {
            // console.log(val);
            if (/^#([a-f]|\d){6}/g.test(val))
                criteriaMeet++;
        }
        else if (key === "ecl") {
            // console.log(val);
